...
class CountryDetailViewController: UIViewController {
    ...
    func setup() {
        ...
        //Map section
        if let borders = countryDetail.borders, borders.count > 0 {
...
extension CountryDetailViewController {
    func topView() -> UIView {
        ...
        let countryLbl = UIView().getLabel(text: countryDetail.nativeName, font: .bold)
        let capitalOfCountry = UIView().getLabel(text: countryDetail.capital ?? "")
        ...
        if let url = URL(string: countryDetail.flags.png) {
    ...
    func mapView() -> UIView {
        ...
        let image = UIView().getImage(content: .scaleAspectFill)
        
        image.image = ImageFromPDF.drawIMGfromPDF(thisPDF: countryDetail.alpha3Code)
        ...
        nstackTime.addArrangedSubview(UIView().getLabel(text: countryDetail.subregion))
        nstackTime.addArrangedSubview(UIView().getLabel(text: "\(countryDetail.area)"))
        nstackTime.addArrangedSubview(UIView().getLabel(text: "(\("\(countryDetail.latlng.first ?? 0.0)" ), \(countryDetail.latlng[1]))" ))
        ...
        image.heightAnchor.constraint(equalTo: contentImg.heightAnchor, multiplier: 1).isActive = true
        image.widthAnchor.constraint(equalTo: contentImg.heightAnchor, multiplier: 1).isActive = true
    ...
    func peopleView() -> UIView {
        ...
        population.addArrangedSubview(iconLbl(lblText: "\(countryDetail.population)", img: ImageFromPDF.drawIMGfromPDF(thisPDF: "Population")!))
        
        var languages = ""
        for (index, language) in countryDetail.languages.enumerated() {
            languages = "\(language.name)\(index == countryDetail.languages.count-1 && countryDetail.languages.count > 1 ? "," : "")"
        }
        
        let language = UIView().getStack(axis: .horizontal, aligment: .fill, distribution: .fill)
        language.addArrangedSubview(iconLbl(lblText: languages, img: ImageFromPDF.drawIMGfromPDF(thisPDF: "Languages")!))
        
        var codes = ""
        for (index, code) in countryDetail.callingCodes.enumerated() {
            codes = "\(code)\(index == countryDetail.callingCodes.count-1 && countryDetail.callingCodes.count > 1 ? "," : "")"
        }
        
        let codePhone = UIView().getStack(axis: .horizontal, aligment: .fill, distribution: .fill)
        codePhone.addArrangedSubview(iconLbl(lblText: codes, img: ImageFromPDF.drawIMGfromPDF(thisPDF: "PhoneCode")!))
        ...
        let image = UIImageView(image: ImageFromPDF.drawIMGfromPDF(thisPDF: "Timezones")!)
        ...
        countryDetail.timezones.forEach { time in
            let withoutCode = time.components(separatedBy: "-")
            if withoutCode.indices.contains(1){
                let lbl = UIView().getLabel(text: "-\(withoutCode[1])")
    ...
    func exchangeView() -> UIView {
        ...
        let hstack = iconLbl(lblText: getSymbol(countryDetail.currencies.first?.code ?? "") ?? "", img:  ImageFromPDF.drawIMGfromPDF(thisPDF: "Currency")!)
...
extension CountryDetailViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return countryDetail.borders?.count ?? 0
    }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.configCell(border: countryDetail.borders?[indexPath.item] ?? "")
